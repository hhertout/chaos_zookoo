...
// Strategy controls how matching pods are removed.
...
func ParseConfig(data []byte) (Config, error) {
	...
	if cfg.Metadata.Name == "" {
		return Config{}, fmt.Errorf("killing config requires metadata.name")
	}
	if cfg.Metadata.Namespace == "" {
		return Config{}, fmt.Errorf("killing config requires metadata.namespace")
	}
	if cfg.Scenario.Matchers.IsEmpty() {
		return Config{}, fmt.Errorf("killing config requires at least one scenario.matchers entry")
	}
	if cfg.Scenario.MinAvailable < 0 {
		return Config{}, fmt.Errorf("killing scenario.minAvailable must be >= 0")
	}

	switch {
	case cfg.Scenario.RawInterval != "" && cfg.Scenario.RawCron != "":
		return Config{}, fmt.Errorf("killing config: scenario.interval and scenario.cron are mutually exclusive")
	...
	case cfg.Scenario.RawInterval != "":
		...
		if dur <= 0 {
			return Config{}, fmt.Errorf("killing scenario.interval must be > 0, got %s", cfg.Scenario.RawInterval)
		}
		cfg.interval = dur
	default:
		return Config{}, fmt.Errorf("killing config requires scenario.interval or scenario.cron")
	}

	if cfg.Scenario.RawWait != "" {
		if cfg.cronExpr != "" {
			return Config{}, fmt.Errorf("killing scenario.wait is not supported with cron scheduling")
		}
		...
		if w < 0 {
			return Config{}, fmt.Errorf("killing scenario.wait must be >= 0")
		}
		if w >= cfg.interval {
			return Config{}, fmt.Errorf("killing scenario.wait (%s) must be < scenario.interval (%s)", w, cfg.interval)
		}
		cfg.wait = w
	}
